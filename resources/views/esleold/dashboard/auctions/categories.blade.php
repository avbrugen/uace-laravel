...
@section('content')
    ...
    <div class="container-fluid">
        <h2 class="sub-header">Категории аукционов</h2>
...
<ul>
    @foreach($cats as $category)
        <li>{{ $category->name }} <!--<a class="category_edit" href="#" data-category-id="{{ $category->id }}" data-toggle="modal" data-target="#modalEdit">Edit</a>--></li>
        <ul>
            @foreach($category->getDescendants() as $par)
                <li>{{ $par->name }} <a class="category_edit" href="#" data-category-id="{{ $par->id }}" data-toggle="modal" data-target="#modalEdit">Изменить</a></li>
            @endforeach
        </ul>
    @endforeach
</ul>
